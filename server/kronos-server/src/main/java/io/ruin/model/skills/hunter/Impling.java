...
public enum Impling {
	...
	BABY(		1635, 	17,	27,	18,	 20, 	11238, 	5,	 	5),
	YOUNG(		1636, 	22,	32,	20,	 22, 	11240, 	5,	 	5),
	GOURMET(	1637, 	28,	38,	22,	 24, 	11242, 	100, 	20),
	EARTH(		1638, 	36,	46,	25,	 27, 	11244, 	5,	 	5),
	ESSENCE(	1639, 	42,	52,	27,	 29, 	11246, 	5,	 	5),
	ECLECTIC(	1640, 	50,	60,	30,	 32, 	11248, 	75, 	30),
	NATURE(		1641, 	58,	68,	34,	 36, 	11250, 	100, 	30),
	MAGPIE(		1642, 	65,	75,	44,	 54, 	11252, 	75,	 	25),
	NINJA(		1643, 	74,	85,	50,	 60, 	11254, 	60,	 	30),
	DRAGON(		1644, 	83,	93,	65,	 75, 	11256, 	12,	 	12),
	LUCKY(		7233, 	89,	99,	0,		 0, 	19732, 	4, 		4);
	...
	private static void attemptCatch(Player player, NPC npc, Impling impling) {
		...
		player.startEvent(event -> {
			...
			event.delay(2);
			player.unlock();
			int levelsHigherThanReq = player.getStats().get(StatType.Hunter).currentLevel - levelReq;
			int percentChance = (int) (50 + (1.4 * levelsHigherThanReq));
			if (Random.rollPercent(percentChance)) {
				...
				switch (impling.npcId) {
					case NpcID.MAGPIE_IMPLING:
					case NpcID.MAGPIE_IMPLING_1652:
						DailyTasks.handleTaskDecrement(player, "magpie");
						break;
					case NpcID.ECLECTIC_IMPLING:
					case NpcID.ECLECTIC_IMPLING_1650:
						DailyTasks.handleTaskDecrement(player, "eclectic");
						break;
					case NpcID.NATURE_IMPLING:
					case NpcID.NATURE_IMPLING_1651:
						DailyTasks.handleTaskDecrement(player, "nature");
						break;
					case NpcID.GOURMET_IMPLING:
					case NpcID.GOURMET_IMPLING_1647:
						DailyTasks.handleTaskDecrement(player, "gourmet");
						break;
	...
	private static final int PURO_PURO_STATIC_RESPAWN_DELAY = 30; // respawn time for static spawns in puro puro (baby, young gourmet, earth, eclectic)
	private static final int PURO_PURO_RANDOM_SPAWN_DELAY = 10; // respawn time for random spawns in puro puro (other implings)
	private static final int PURO_PURO_MAX_RANDOM_IMPLINGS = 70; // maximum number of random spawns that can be active at a time
	...
	private static final int OVERWORLD_MAX_IMPLINGS = 80; // maximum number of overworld spawns that can be active at one time
...
//				});
//			}
			// Ticket: 405 | when a nature, magpie, ninja, dragon or lucky impling is caught, and it should only spawn one of those 5 in the world.
			if (type == NATURE || type == MAGPIE || type == NINJA || type == DRAGON || type == LUCKY) {
				var implings = List.of(NATURE, MAGPIE, NINJA, DRAGON, LUCKY);
				var spawnPoint = npc.getSpawnPosition();
				npc.remove();
				var newImpling = Random.get(implings);
				assert newImpling != null : "Impling to spawn was null";
				World.startEvent(PURO_PURO_STATIC_RESPAWN_DELAY, e ->
					new NPC(newImpling.npcId).spawn(spawnPoint, 12));
			}
			// Ticket: 405 | If an impling is caught, that same impling should respawn from the one caught, not a random impling
			else
				npc.addEvent(event -> {
					npc.setHidden(true);
					event.delay(PURO_PURO_STATIC_RESPAWN_DELAY);
	...
	private static Impling getRandomPuroPuroSpawn() {
		...
		for (Impling impling : values()) {
			...
			if (roll <= 0) {
				log.trace("Landed on impling {}", impling);
				return impling;
			}
		}
	...
	private static void spawnRandomImplingPuroPuro() {
		...
		Position spawnPosition = PURO_PURO.randomPosition();
		...
		log.trace("Spawning impling type {} ({}) at {} now a  total of {}", type, impling.getName(), spawnPosition, ACTIVE_PURO_PURO_IMPLINGS);
	}
	...
	private static final Position[] OVERWORLD_RANDOM_SPAWN_POSITIONS = {
		...
		new Position(3146, 3496, 0), //home spawns
		new Position(3153, 3509, 0),
		new Position(3168, 3510, 0),
		new Position(3181, 3502, 0),
		new Position(3184, 3491, 0),
		new Position(3172, 3470, 0),
	...
	public static void register() {
		for (Impling impling : values()) {
			...
			NPCType.get(impling.npcId).flightClipping = true;
		}

		//puro-puro random spawns
		World.startEvent(event -> {
			//spawn a few on startup
			while (ACTIVE_PURO_PURO_IMPLINGS < PURO_PURO_MAX_RANDOM_IMPLINGS)
