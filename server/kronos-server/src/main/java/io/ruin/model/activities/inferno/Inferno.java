...
public class Inferno {
	...
	public static void register() {
		NPCType def = NPCType.get(PILLAR_ALIVE);
		...
		NPCType.get(NIBBLER).ignoreOccupiedTiles = true;

		def = NPCType.get(SHIELD);
		def.combatHandlerClass = PassiveCombat.class;
		def.combatInfo = new npc_combat.Info();
		def.combatInfo.hitpoints = 800;
	...
	private boolean[] deadPillars = new boolean[3];

	@Getter
	private transient DynamicMap map;
	@Getter
	private transient Player player;
	private transient List<Pillar> pillars;
	private transient ActivityTimer timer;
	private transient int spawnOffset;
	private transient boolean logoutRequest;
	private transient boolean killedZuk;
	private transient boolean preparingWave;
	...
	public boolean allPillarsAlive() {
		return pillars.stream().allMatch(p -> p.npc.getHp() > 0 && pillars.size() == 3);
	}
	...
	public void start(boolean login) {
		...
		player.startEvent(e -> {
			...
			} catch (DynamicMap.DynamicMapBuildException ex) {
			...
			if (wave < 67) {
				if (!deadPillars[0])
					pillars.add(new Pillar(30353, map.convertX(2257), map.convertY(5349),
							VarPlayerRepository.INFERNO_PILLAR_DAMAGE_WEST));
				if (!deadPillars[1])
					pillars.add(new Pillar(30354, map.convertX(2267), map.convertY(5335),
							VarPlayerRepository.INFERNO_PILLAR_DAMAGE_SOUTH));
				if (!deadPillars[2])
					pillars.add(new Pillar(30355, map.convertX(2274), map.convertY(5351),
							VarPlayerRepository.INFERNO_PILLAR_DAMAGE_EAST));
			}
			...
			if (login) {
				...
				e.delay(5);
				player.dialogue(new MessageDialogue("You jump into the fiery cauldron of The Inferno; your heart is pulsating.")
						.lineHeight(24).hideContinue());
				e.delay(2);
	...
	private void stop(boolean logout) {
		// Clear NPCs first to prevent any NPEs
		if (map != null && map.getNpcs() != null)
			getMap().getNpcs().forEach(NPC::remove);
		...
		if (logout) {
			...
			if (lastWave == 0) {
				...
				if (!killedZuk) {
					...
					player.dialogue(new NPCDialogue(HOST,
					...
					if (player.activePerksList.isEmpty()) {
						Objects.requireNonNull(player.combatAchievementsList
								.get(player.getCombatAchievementIndexByOrdinal(CombatAchievements.PERKLESS_INFERNO.ordinal()))
								.getCombatAchievement()).check(player);
					}
					player.infernoBestTime = timer.stop(player, player.infernoBestTime);
					if (ActivityTimer.timeInSeconds(player.infernoBestTime) < 600) {
						Objects.requireNonNull(player.combatAchievementsList
								.get(player.getCombatAchievementIndexByOrdinal(CombatAchievements.INFERNO_SPEED_RUNNER.ordinal()))
								.getCombatAchievement()).check(player);
					}
					if (!pillarsFellBeforeWave67) {
						Objects.requireNonNull(player.combatAchievementsList
								.get(player.getCombatAchievementIndexByOrdinal(CombatAchievements.NIBBLERS_BEGONE.ordinal()))
								.getCombatAchievement()).check(player);
					}
					player.getInventory().addOrDrop(21295, 1);
					player.getInventory().addOrDrop(6529, 16440);
					Broadcast.GLOBAL.sendNews(player,
							"Very impressive for a JalYt. " + player.getName() + " has even managed to defeat the Inferno!");
					player.addToCollectionLog(new Item(21295, 1));
				}
			}
			player.inferno = null;
	...
	private void prepareWave(boolean login) {
		...
		if (!login)
			wave++;
		map.addEvent(event -> {
			event.setCancelCondition(() -> map == null || player == null);
			if (!practice)
	...
	boolean pillarsFellBeforeWave67 = false;
	...
	private void beginWave() {
		...
		if (wave == 67) { // Single jad
			// Collapse pillars
			if (allPillarsAlive()) {
				pillarsFellBeforeWave67 = false;
			} else
				pillarsFellBeforeWave67 = true;
	...
	private void spawnZuk() {
		...
		player.addEvent(event -> {
			...
			event.delay(2);
			player.lock();
			// Need to teleport the player to z 1 because we have to change a few objects
			// there
			player.currentDynamicMap = map;
			player.inDynamicMap = true;
			...
			shield.addEvent(e -> {
				e.delay(3);
				shield.step(0, -1, StepType.WALK);
				e.delay(1);
				shield.step(0, -1, StepType.WALK);
			});
			player.dialogue(new NPCDialogue(7690,
					"Oh no! TzKal-Zuk's prison is breaking down. This not meant to have happened. There's nothing I can do for you now JalYt!")
							.animate(615).hideContinue());
			event.delay(9);
			player.closeDialogue();
			player.getPacketSender().resetCamera();
			player.unlock();
	...
	int wave68JadKills = 0;
	boolean zukJadKilled = false;
	...
	private void handleDeath(NPC npc) {
		...
		if (wave == 68) {
			if (npc.getId() == JAD) {
				...
				if (wave68JadKills == 3) {
					Objects.requireNonNull(player.combatAchievementsList
							.get(player.getCombatAchievementIndexByOrdinal(CombatAchievements.PLAYING_WITH_JADS.ordinal()))
							.getCombatAchievement()).check(player);
				}
				World.startEvent(e -> {
					e.delay(25);
		...
		if (npc.getId() == 7706) {
			if (!zukJadKilled) {
				Objects.requireNonNull(player.combatAchievementsList
						.get(player.getCombatAchievementIndexByOrdinal(CombatAchievements.JAD_WHAT_ARE_YOU_DOING_HERE.ordinal()))
						.getCombatAchievement()).check(player);
			}
			killedZuk = true;
			player.currentDynamicMap = null;
		...
		if (map.getNpcs().stream()
				.allMatch(n -> n.getId() == PILLAR_ALIVE || n.getId() == PILLAR_DEAD || n.getCombat().isDead())) {
			if (!finishPractice()) {
				if (logoutRequest) {
	...
	private void spawnBloblets(NPC npc) {
		...
		Bounds b = new Bounds(npc.getPosition(), npc.getPosition().relative(npc.getSize(), npc.getSize()),
	...
	private boolean allowLogout() {
		...
		if (!logoutRequest) {
			player.sendMessage(
					"<col=ef1020>Your logout request has been received. The minigame will be paused at the end of this wave.<br><col=ef1020>If you try to log out before that, you will have to repeat this wave.");
			logoutRequest = true;
			return false;
		}
		return true;
	...
	private final class Pillar {
		...
		public Pillar(int objId, int x, int y, VarPlayerRepository config) {
			...
			NPCType.get(PILLAR_ALIVE).ignoreMultiCheck = true;
			NPCType.get(PILLAR_DEAD).ignoreMultiCheck = true;
		}
