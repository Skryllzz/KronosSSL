...
//TODO: add a real interface defs with components
public class InterfaceDef {
	...
	public static void printConfigs(int interfaceId, boolean recursiveSearch) {
		...
		for (ScriptDef script : scripts) {
			for (int i = 0; i < script.instructions.length; i++) {
				...
				if (instruction == 1)
					...
					varpIds.add(VarBitType.get(script.intOperands[i]).varpId);
			}
		}
		if (varpIds.isEmpty()) {
		...
		System.out
			.println("Interface " + interfaceId + " uses the following configs: [recursiveSearch=" + recursiveSearch + "]");
		System.out.println("    scripts=" + scripts.stream().map(def -> def.id).collect(Collectors.toList()).toString());
	}
				...
				for (int x = 0; x < scriptListeners; x++) {
					try {
						...
						for (int i = 0; i < length; i++) {
							...
							if (type == 1) {
								strCount++;
								objects[i] = in.readStringCp1252NullTerminated();
								continue;
							}
							continue c;
						}
						...
						if (script == null || intCount - 1 != script.intArgumentCount || strCount != script.stringArgumentCount)
							continue c;
						...
						for (int x = 0; x < argCount; x++) {
							switch (script.instructions[offset]) {
								...
								case 4000: // ???
									...
									break;
								default:
									System.out.println("Script (" + script.id + ") has Unhandled Instruction: [" + (offset + 1) + "]="
										+ script.instructions[offset]);
									continue s;
							}
							offset--;
						}
