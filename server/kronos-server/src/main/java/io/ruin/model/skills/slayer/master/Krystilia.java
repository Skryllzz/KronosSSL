...
public class Krystilia {
	...
	private static void assignTask(Player player) {
		...
		VarPlayerRepository.SLAYER_MASTER.set(player, SlayerMaster.KRYSTILIA_ID);
										...
										player.dialogue(new PlayerDialogue("Wow, can you teach me?"),
											...
											new PlayerDialogue("Okay, what's first?").action(() -> {
												...
												if (task != null) {
													...
													player.dialogue(new NPCDialogue(KRYSTILIA, "We'll start you off hunting " + SlayerUnlock.taskName(player, task.getUid()) + ", you'll need to kill " + num + "<br>of them."));
												}
											}));
	...
	private static void giveTask(Player player) {
		...
		if (task.equals(SlayerCreature.BOSSES)) {
			player.dialogue(
				new NPCDialogue(KRYSTILIA, "Your new task is to kill " + SlayerUnlock.taskName(player, task.getUid()) +
					" in the wilderness. How many would you like to slay?").onDialogueOpened(() -> {
					player.integerInput("How many would you like to slay? (3-35)", (i) -> {
						...
						player.dialogue(
							new NPCDialogue(KRYSTILIA, "Excellent. You're now assigned to kill " + SlayerUnlock.taskName(player, task.getUid()) + " boss " + i + " times in the wilderness."),
							new OptionsDialogue(new Option("Got any tips for me?", () -> player.dialogue(
								new PlayerDialogue("Got any tips for me?"),
								new NPCDialogue(KRYSTILIA, SlayerUnlock.tipForWilderness(task)),
			...
			player.dialogue(
				new NPCDialogue(KRYSTILIA, "Excellent, you're doing great. Your new task is to kill " + left + " " + SlayerUnlock.taskName(player, task.getUid()) + " in the wilderness."),
				new OptionsDialogue(
					new Option("Got any tips for me?", () -> player.dialogue(
						new PlayerDialogue("Got any tips for me?"),
						new NPCDialogue(KRYSTILIA, SlayerUnlock.tipFor(task)),
					...
					new Option("I'd like to skip this task. I have " + VarPlayerRepository.SLAYER_POINTS.get(player) + " Slayer points.", () -> {
						...
						if (slayerPointsBeforeSkip >= cost) {
							...
							player.dialogue(new NPCDialogue(KRYSTILIA, "Task skipped. You now have " + slayerPointsAfterSkip + " Slayer points remaining."));
						} else {
							player.dialogue(new NPCDialogue(KRYSTILIA, "Sorry, you don't have enough Slayer points to skip that task."));
						}
					}),
	...
	public static void register() {
		NPCAction.register(KRYSTILIA, 1, (player, npc) -> handleInteraction(player, 1));
		NPCAction.register(KRYSTILIA, "Trade", (player, npc) -> NewShopHandler.openShop(player, NewShopHandler.slayerStore));
		NPCAction.register(KRYSTILIA, 3, (player, npc) -> handleInteraction(player, 3));
		NPCAction.register(KRYSTILIA, 5, (player, npc) -> handleInteraction(player, 5));
	}
}
