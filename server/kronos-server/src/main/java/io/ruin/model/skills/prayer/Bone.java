...
public enum Bone {
	...
	RAURG_BONES(4832, 96.0, PlayerCounter.RAURG_BONES_BURIED, PlayerCounter.RAURG_BONES_ALTAR),
	...
	WYRM_BONES(22780, 50.0, PlayerCounter.WYRM_BONES_BURIED, PlayerCounter.WYRM_BONES_ALTAR),
	DRAKE_BONES(22783, 80.0, PlayerCounter.DRAKE_BONES_BURIED, PlayerCounter.DRAKE_BONES_ALTAR),
	HYDRA_BONES(22786, 110.0, PlayerCounter.HYDRA_BONES_BURIED, PlayerCounter.HYDRA_BONES_ALTAR),
	...
	Bone(int id, double exp, PlayerCounter buryCounter, PlayerCounter altarCounter) {
		this.id = id;
		this.notedId = ObjType.get(id).notedId;
		;
		this.exp = exp;
	...
	public void bury(Player player, Item bone) {
		player.resetActions(true, false, true);
		player.startEvent(event -> {
			if (player.boneBuryDelay.isDelayed())
				return;
			ObjType neckDef = player.getEquipment().getDef(Equipment.SLOT_AMULET);
			if (neckDef != null && (neckDef.id == ItemID.DRAGONBONE_NECKLACE || neckDef.id == ItemID.BONECRUSHER_NECKLACE)) {
				boneNecklaceEffect(player, bone);
			}
			if (player.getPosition().inBounds(LAVAMAZE) && bone.getId() == LAVA_DRAGON_BONES.id) {
//                player.sendMessage("This is a test mf");
				DailyTasks.handleTaskDecrement(player, "lavadragisle");
				player.getStats().addXp(StatType.Prayer, exp * 1, true);
			}
			bone.remove();
			player.animate(827);
	...
	public static void register() {
		for (Bone bone : values()) {
			if (bone.exp <= Bone.BIG_BONES.exp)
				ObjType.get(bone.id).allowFruit = true;
