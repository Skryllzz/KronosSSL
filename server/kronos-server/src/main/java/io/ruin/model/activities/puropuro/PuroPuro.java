...
public class PuroPuro {
	...
	private static void exit(Player player, GameObject portal) {
		player.startEvent(event -> {
			player.lock();
			player.stepAbs(portal.x, portal.y, StepType.WALK);
			event.delay(2);
			player.animate(6601);
	...
	public static void register() {
		...
		ObjectAction.register(24991, "enter", (player, obj) -> player.startEvent(event -> {
			...
			player.graphics(1118);
			event.delay(9);
			player.getMovement().teleport(ServerTeleports.PURO_PURO.getTeleportPos());
			player.unlock();
		}));
			...
			while (true) {
				...
				for (WheatPosition wp : POSITIONS) {// todo make this run only when there are players in the area
					...
					if (!Random.rollPercent(30))
						continue;
					if (obj == null || (obj.getId() != WHEAT_STALE_HORIZONTAL && obj.getId() != WHEAT_STALE_VERTICAL)) {
						if (obj != null)
							obj.remove();
