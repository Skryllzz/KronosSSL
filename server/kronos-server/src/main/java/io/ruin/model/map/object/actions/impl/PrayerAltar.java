...
public class PrayerAltar {
	...
	public static void register() {
		...
		ObjectAction.register(18258, "Pray-at", ((player, obj) -> pray(player)));
		/**
		 * Custom Edgeville altar
		 */
		final int[] ALTARS = new int[]{31858, 39642};
		...
		for (Bone bone : Bone.VALUES) {
			...
			ItemObjectAction.register(bone.id, 409, (player, boneItem, obj) -> {
				if (player.getInventory().hasMultiple(boneItem.getId())) {
					SkillDialogue.make(player, item);
					return;
				}
			...
			ItemObjectAction.register(bone.id, 14860, (player, boneItem, obj) -> {
				if (player.getInventory().hasMultiple(boneItem.getId())) {
					SkillDialogue.make(player, item);
					return;
				}
			...
			ItemObjectAction.register(bone.id, 2640, (player, boneItem, obj) -> {
				if (player.getInventory().hasMultiple(boneItem.getId())) {
					SkillDialogue.make(player, item);
					return;
				}
