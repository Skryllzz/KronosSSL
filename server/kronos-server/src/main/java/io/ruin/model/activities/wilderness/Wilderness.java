...
import static io.ruin.model.map.object.actions.ObjectAction.log;
...
public class Wilderness {
	...
	private static void entered(Player player) {
		...
		VarPlayerRepository.IN_WILDERNESS.set(player, 1);
		VarPlayerRepository.BLIGHTED_SACK_ENABLED.set(player, 1);
		players.add(player);
	...
	private static void openWildernessOverlay(Player player) {
		...
		VarPlayerRepository.IN_PVP_AREA.set(player, 1);
		// hide safe area sprite
		player.getPacketSender().setHidden(Interface.WILDERNESS_OVERLAY, 47, true);
		// show wilderness level
		player.getPacketSender().setHidden(Interface.WILDERNESS_OVERLAY, 53, false);
	}
	...
	private static void exited(Player player, boolean logout) {
		...
		if (!logout) {
			...
			player.chaosFanaticKillsSincePotion = 0;
			...
			player.getEquipment().update(0); // force a slot update
			player.getEquipment().sendUpdates();
			VarPlayerRepository.IN_WILDERNESS.set(player, 0);
			VarPlayerRepository.BLIGHTED_SACK_ENABLED.set(player, 0);
		}
	}
	...
	private static boolean allowAttack(Player player, Player pTarget, boolean message) {
		if (player.wildernessLevel == 0) {
			if (message)
				player.sendMessage("You can't attack players while you are outside the Wilderness.");
			return false;
		}
		...
		if (pTarget.getIpInt() == player.getIpInt() && !World.isDev()) {
			player.sendMessage("You can't attack yourself inside the wilderness!");
			return false;
		}
		...
		if (!player.inMulti() && !pTarget.inMulti()) {
			...
			if (player.riskedBloodMoney < pTargetProtection) {
				player.sendMessage(
						Color.DARK_RED.wrap("This player is currently under risk protection. You must risk an additional " +
								NumberUtils.formatNumber(pTargetProtection - player.riskedBloodMoney)
								+ " coins in order to attack them."));
				return false;
			}
		...
		if (!isWithinCombatRange(combatLevel, wildernessLevel, targetCombatLevel, targetWildernessLevel)) {
			if (message)
				player.sendMessage("Your combat level difference is to high to attack from here.");
			return false;
		}
	...
	/**
	 * Determines if two players are within combat range of each other
	 * based on their combat levels and wilderness levels.
	 *
	 * @param combatLevel the combat level of the first player
	 * @param wildernessLevel the wilderness level of the first player
	 * @param targetCombatLevel the combat level of the target player
	 * @return {@code true} if the players are within combat range; {@code false} otherwise
	 */
	private static boolean isWithinCombatRange(
		int combatLevel,
		int wildernessLevel,
		int targetCombatLevel,
		int targetWildernessLevel
	) {
		var minTargetLevel = combatLevel - wildernessLevel;
		var maxTargetLevel = combatLevel + wildernessLevel;

		var minPlayerLevel = targetCombatLevel - targetWildernessLevel;
		var maxPlayerLevel = targetCombatLevel + targetWildernessLevel;

		return (minTargetLevel <= targetCombatLevel && targetCombatLevel <= maxTargetLevel)
			&& (minPlayerLevel <= combatLevel && combatLevel <= maxPlayerLevel);
	}


	/**
	 * Checks if the {@code player}'s remote ip address has 2 or more players
	...
	private static void setLevel(int absX, int absY, int absZ, int level) {
		if (level > 255) {
			log.error("max wildy level is 255 not {}", level);
		}
		Tile.get(absX, absY, absZ, true).wildernessLevel = (byte) level;
	}
	 ...
	 * bounds.forEachPos(pos -> setSafePVPInstance(pos.getX(), pos.getY(),
	 * pos.getZ(), safe));
	 * }
	 * 
	 * private static void setSafePVPInstance(int absX, int absY, int absZ, boolean
	 * safe) {
	...
	public static void register() {
		...
		setLevels(new Bounds(2944, 9920, 3454, 10367, -1), y -> ((y - 9920) / 8) + 1); // idk
		...
		ItemAction.registerInventory(30141, "read", (player, item) -> {
			...
			player.sendMessage("You now have " + player.blackChinchompaBoost.remainingToMins()
					+ " minutes of Black chinchompa skilling boost.");
		});
		ItemAction.registerInventory(30143, "read", (player, item) -> {
			...
			player.getInventory().remove(item);
			player.sendMessage(
					"You now have " + player.darkCrabBoost.remainingToMins() + " minutes of Dark crab skilling boost.");
		});
	...
	public static void rollPvPItemDrop(Player player, NPC npc, Position dropPosition) {
		...
		if (Random.rollDie(chance)) {
			...
			player.sendMessage("You have been skulled and tele-blocked because of your loot!");
	...
	public static boolean stopFollowing(Player player, int stepX, int stepY) {
		if (stopFollowCheck(player)) {
			...
			boolean check = (player.getMovement().following != null) || (route != null && route.target != null
					&& route.target.player != null && route.target != player.getCombat().getTarget());
			if (check && ((player.pvpInstancePosition != null)
					|| (player.wildernessLevel == 0 && getLevel(stepX, stepY, player.getHeight()) > 0))) {
