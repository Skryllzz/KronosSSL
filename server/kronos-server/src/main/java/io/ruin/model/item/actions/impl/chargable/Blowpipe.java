...
public class Blowpipe {
	...
	public static void register() {
		ItemAction.registerInventory(12922, "dismantle", SerpentineHelm::dismantle);
	...
	private static void check(Player player, Item blowpipe) {
		...
		if (dart == Dart.NONE)
			...
			darts = ObjType.get(dart.id).name + " x " + getDartAmount(blowpipe);
		String scales;
...
//        System.out.println("Scales: " + scalesAmount); //prints out 16380 here
	...
	private static void load(Player player, Item blowpipe, Item dartItem, Dart dart) {
		if(dartItem.getId() == 33011) {
			player.sendMessage("This weapon cannot hold these kind of darts.");
			return;
		}
		...
		if (dart == Dart.IRON || dart == Dart.BRONZE || dart == Dart.STEEL || dart == Dart.BLACK) {
			player.sendMessage("You can't use that kind of dart.");
			return;
		}
	...
	public static void update(Item blowpipe, Dart dart, int dartAmount, int scalesAmount) {
		...
		if (AttributeExtensions.hasAttribute(blowpipe, FULLY_LOADED)) {
			int level = AttributeExtensions.getCharges(FULLY_LOADED, blowpipe);
			int chance = 5 - level;
	...
	public static Dart getMagmaOrDart(Item blowpipe) {
		if (blowpipe.getId() == 28688)
			return Blowpipe.Dart.AMETHYST;
		return Dart.values()[blowpipe.getAttributeInt(AttributeTypes.AMMO_ID, 0)];
	}
