 ...
 * TODO(polish) - this is so retarded i can't even.
 */
@Slf4j
@ToString
public class Item {
	...
	@Setter
	@Getter
	private transient ItemContainerG container;
	@Setter
	private transient int slot = -1;

	public transient Broadcast lootBroadcast;
	...
	/**
	 * Creates an <code>Item</code> with the specified id, with an amount of 1 and
	 * no attributes.
	 *
	 * @param id The item id.
	 */
	 ...
	 * @return
	 */
	public ObjType getDef() {
		return ObjType.cached.get(id);
	}
	 ...
	 *
	 * @param key The attribute key
	 * @return the attribute value, or <code>defaultValue</code> if attribute key
	 *         has no value
	 */
	 ...
	 */
	public int incrementAmount(long amount) {
		var newAmount = (long) this.amount + amount;
		if (newAmount <= 0) {
			this.amount = 0;
			remove();
			return 0;
	...
	public int move(int addId, int amount, ItemContainerG<?> toContainer) {
		...
		if (hash == 0) {
			...
			// Never allow more than one to be moved at a time. This should only ever apply
			// to banking.
			// Example: Removing "All" blowpipes when you have 3 in your bank with all
			// different charges.
			// Since they all have their own unique value, they'll never stack in the bank.
			// So withdrawing
			// just 1 at a time kind of makes sense anyways.. (And making it work the other
			// way = slower code!)
			stack = false;
	...
	public void examine(Player player) {
		...
		if (def == null)
			return;
		String examine = def.examine == null ? "This item has no examine" : def.examine;
		if (amount >= 100000)
			examine = "" + StringUtils.formatNumber(amount) + "x " + def.name + ".";
		if (id == 32000) {
		...
		if (!ItemUpgradeInterface.getItemAttachments(this).equalsIgnoreCase("None")) {
			String attachments = ItemUpgradeInterface.getItemAttachmentsComma(this);
			player.sendMessage("Item Perks: " + attachments);
		}
