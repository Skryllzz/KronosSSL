...
public enum Armour {
	...
	STUDDED_BODY(41, ArmourType.STUDDED_BODY, 1133, "a studded body", 40.0, 1),
	STUDDED_CHAPS(44, ArmourType.STUDDED_CHAPS, 1097, "a pair of studded chaps", 42.0, 1),
	...
	private static void craft(Player player, Armour armourType, int amount) {
		...
		ArmourType armour = armourType.leatherType;
		...
		if (maxAmount < amount)
			amount = maxAmount;
		if (amount < 1) {
			player.dialogue(new MessageDialogue("You need at least %d pieces of %s to make %s."
				.formatted(armourType.required, armourType.leatherType.leatherName, armourType.leatherName)));
			return;
		}
		...
		player.startEvent(event -> {
			int made = 0;
			while (made++ < amt) {
				var makingStuddedLeather = armour == ArmourType.STUDDED_CHAPS || armour == ArmourType.STUDDED_BODY;
				Item thread = player.getInventory().findItem(THREAD);
				Item needle = player.getInventory().findItem(NEEDLE);
				if (!makingStuddedLeather) {
					if (thread == null) {
						player.dialogue(new MessageDialogue("You need some thread to make anything out of %s."
							.formatted(armourType.leatherType.leatherName)));
						return;
					}
					if (needle == null) {
						player.dialogue(new MessageDialogue("You need a needle to work with %s."
							.formatted(armourType.leatherType.leatherName)));
						return;
					}
					if ((made + 1) % 5 == 0) {
				...
				int newAmount = armourType.required;
				...
				PerkTaskHandler.handleGatherResource(player, armourType.cutID, 1);
				DailyTasks.handleItemObtained(player, armourType.cutID, StatType.Crafting);
				// player.animate(1249);
				player.sendFilteredMessage("You make %s.".formatted(armourType.leatherName));
				double xpToGain = armourType.exp;
				float multiplier = 1;
				if (player.getPlayerPerkHandler().getActivePerks(player).contains(Perks.ARTS_N_CRAFTS)) {
					int perkIndex = player.getPlayerPerkHandler().getActivePerkIndex(player, Perks.ARTS_N_CRAFTS);
					ArtsnCrafts c = (ArtsnCrafts) player.getPlayerPerkHandler().
	...
	public static void register() {
		 ...
		 * Studded leather
		 */
		ItemItemAction.register(ItemID.LEATHER_BODY, ItemID.STEEL_STUDS, (player, needle, hardLeather) -> SkillDialogue.make(player,
			new SkillItem(STUDDED_BODY.cutID).addAction((p, amount, event) -> craft(p, STUDDED_BODY, amount))));

		ItemItemAction.register(ItemID.LEATHER_CHAPS, ItemID.STEEL_STUDS, (player, needle, hardLeather) -> SkillDialogue.make(player,
			new SkillItem(STUDDED_CHAPS.cutID).addAction((p, amount, event) -> craft(p, STUDDED_CHAPS, amount))));
