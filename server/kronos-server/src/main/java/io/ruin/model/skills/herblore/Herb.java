...
import static io.ruin.model.skills.Tool.*;
...
public enum Herb {
	...
	public static void register() {
		for (Herb herb : values()) {
			String herbName = ObjType.get(herb.cleanId).name.toLowerCase();
			...
			ItemItemAction.register(265, VIAL_OF_BLOOD, (player, herbItem, vialItem) -> {
				if (!player.getStats().check(StatType.Herblore, 80, VIAL_OF_BLOOD, 265, "mix this"))
					return;
				if (player.getInventory().hasMultiple(265, VIAL_OF_BLOOD)) {
					SkillDialogue.make(player, skillItem);
					return;
				}
				herb.mix(player, herbItem, vialItem);
			});
			ItemItemAction.register(5974, Tool.HAMMER, (player, primary, secondary) -> {
				player.startEvent(event -> {
					int amount = player.getInventory().getAmount(primary.getId());
					while (amount-- > 0) {
						if (!player.getInventory().contains(primary.getId())) {
							player.sendMessage(
								"You have ran out of coconuts.");
							return;
						}
						player.getInventory().remove(primary.getId(), 1);
						player.getInventory().addOrDrop(5976, 1);
						player.sendFilteredMessage("You break the coconut with the hammer.");
						event.delay(3);
					}
				});
			});

			ItemItemAction.register(5976, VIAL, (player, primary, vialItem) -> {
				player.startEvent(event -> {
					int amount = player.getInventory().getAmount(primary.getId());
					int vialAmount = player.getInventory().getAmount(vialItem.getId());
					while (amount-- > 0) {
						while (vialAmount-- > 0) {
							player.getInventory().remove(primary.getId(), 1);
							player.getInventory().remove(vialItem.getId(), 1);
							player.getInventory().addOrDrop(5935, 1);
							player.sendFilteredMessage("You squeeze the coconut milk into the vial.");
							event.delay(3);
						}
					}
				});
