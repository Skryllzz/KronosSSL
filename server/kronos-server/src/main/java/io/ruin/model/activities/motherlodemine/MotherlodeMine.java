...
public class MotherlodeMine { //why do we have two motherlode mine classes? Remove the one from skilling and add the actions here
	...
	public static void register() {
		...
		for (int x = 0; x < 64; x++) { // the veins default state is depleted, so we have to restore them all
			for (int y = 0; y < 64; y++) {
				...
				if (obj != null && obj.getId() >= 26665 && obj.getId() <= 26668)
					obj.setId(obj.getId() - 4);
			}
		}
		...
		Tile.getObject(33732, 1233, 2599, 2).skipReachCheck = p -> p.equals(1233, 2602) || p.equals(1235, 2602) || p.equals(1234, 2602);
		Tile.getObject(8150, 1260, 2599, 2).skipReachCheck = p -> p.equals(1260, 2601) || p.equals(1261, 2601) || p.equals(1262, 2600) || p.equals(1262, 2599) || p.equals(1261, 2598) || p.equals(1260, 2598) || p.equals(1259, 2599) || p.equals(1259, 2600);
	...
	private static final void lootSack(Player player) {
		int inSack = VarPlayerRepository.PAY_DIRT_IN_SACK.get(player);
		if (inSack == 0) {
		...
		while (!player.getInventory().isFull() && VarPlayerRepository.PAY_DIRT_IN_SACK.get(player) > 0) {
			...
			VarPlayerRepository.PAY_DIRT_IN_SACK.set(player, VarPlayerRepository.PAY_DIRT_IN_SACK.get(player) - 1);
			PlayerCounter.CLEANED_PAYDIRT.increment(player, 1);
		}

		if (VarPlayerRepository.PAY_DIRT_IN_SACK.get(player) <= 0)
			player.dialogue(new ItemDialogue().one(PAY_DIRT, "You collect your ore from the sack. The sack is now empty."));
		else
			player.dialogue(new ItemDialogue().one(PAY_DIRT, "You collect your ore from the sack. You have " + VarPlayerRepository.PAY_DIRT_IN_SACK.get(player) + " ores remaining."));
	}
	...
	public static boolean isActive() {
		return WHEEL_1.getId() == WORKING_WHEEL || WHEEL_2.getId() == WORKING_WHEEL;
	}
	...
	public static void addToHopper(Player player) {
		...
		if (VarPlayerRepository.PAY_DIRT_IN_SACK.get(player) >= (player.motherlodeBiggerSackUnlocked ? 162 : 81)) {
	...
	private static void sendPaydirt(Player player, int amount) {
		...
		paydirt.startEvent(event -> {
			...
			while (steps < 11) {
				...
				if (isActive()) {
					steps++;
					paydirt.step(0, -1, StepType.WALK);
				}
				event.delay(1);
			}
			VarPlayerRepository.PAY_DIRT_IN_SACK.set(player, VarPlayerRepository.PAY_DIRT_IN_SACK.get(player) + amount);
			player.paydirtInWater -= amount;
			paydirt.remove();
			if (VarPlayerRepository.PAY_DIRT_IN_SACK.get(player) >= (player.motherlodeBiggerSackUnlocked ? 162 : 81)) {
	...
	private static void strutInteract(Player player, GameObject strut, GameObject wheel) {
		if (strut.getId() == WORKING_STRUT)
			return;
		if ((!player.getInventory().contains(HAMMER, 1) && !player.getInventory().hasId(ItemID.DRAGON_WARHAMMER) && !player.getInventory().hasId(Tool.IMCANDO_HAMMER)) &&
			!player.getEquipment().hasId(Tool.IMCANDO_HAMMER) && player.getEquipment().hasId(ItemID.DRAGON_WARHAMMER)) {
	...
	public static void entered(Player player) {
		player.openInterface(ToplevelComponent.WILDERNESS_OVERLAY, 382);
	}
