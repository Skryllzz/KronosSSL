...
public enum FairyRing { //todo add favorite option
	...
	SINCLAIR_WEST(119, new int[]{1, 3, 2}, new Position(1455, 3658, 0), "Kandarin: Sinclair Mansion (west)", 30),
	...
	public static void teleport(Player player, FairyRing entry, GameObject fairyRing) {
		...
		player.startEvent(event -> {
			player.lock();
			if (!player.isAt(fairyRing.x, fairyRing.y) && fairyRing.getId() != Buildable.SPIRIT_TREE_AND_FAIRY_RING.getBuiltObjects()[0]) { // don't walk through the spirit treel ol
				player.stepAbs(fairyRing.x, fairyRing.y, StepType.WALK);
				event.delay(1);
			}
			player.closeInterface(ToplevelComponent.MAINMODAL);
			player.closeInterface(ToplevelComponent.SIDEMODAL);
			if (entry != null)
				VarPlayerRepository.FAIRY_RING_LAST_DESTINATION.set(player, entry.lastDestination);
			event.delay(1);
	...
	public static void openCombinationPanel(Player player, GameObject fairyRing) {
		...
		player.openInterface(ToplevelComponent.MAINMODAL, Interface.FAIRY_RING_COMBINATION);
		player.openInterface(ToplevelComponent.SIDEMODAL, Interface.FAIRY_RING_TRAVEL_LOG);
	}

	private static void resetCombinationPanel(Player player) {
		VarPlayerRepository.FAIRY_RING_LEFT.set(player, 0);
		VarPlayerRepository.FAIRY_RING_RIGHT.set(player, 0);
		VarPlayerRepository.FAIRY_RING_MIDDLE.set(player, 0);
	}

	private static int leftButton(Player player) {
		return VarPlayerRepository.FAIRY_RING_LEFT.get(player);
	}

	private static int rightButton(Player player) {
		return VarPlayerRepository.FAIRY_RING_RIGHT.get(player);
	}

	private static int middleButton(Player player) {
		return VarPlayerRepository.FAIRY_RING_MIDDLE.get(player);
	}

	private static final int[] FAIRY_RING = {29495, 29560, 29228, 27123};
	...
	public static void register() {
		for (int ring : FAIRY_RING) {
			...
			ObjectAction.register(ring, 3, (player, obj) -> {
				for (FairyRing fairyRing : values()) {
					if (fairyRing.lastDestination == VarPlayerRepository.FAIRY_RING_LAST_DESTINATION.get(player))
		...
		InterfaceHandler.register(Interface.FAIRY_RING_COMBINATION, h -> {
			 ...
			 * Left wheel
			 */
			h.actions[19] = (SimpleAction) p -> VarPlayerRepository.FAIRY_RING_LEFT.set(p, (leftButton(p) == 3 ? 0 : leftButton(p) + 1));
			h.actions[20] = (SimpleAction) p -> VarPlayerRepository.FAIRY_RING_LEFT.set(p, (leftButton(p) == 0 ? 3 : leftButton(p) - 1));
			/**
			 * Middle wheel
			 */
			h.actions[21] = (SimpleAction) p -> VarPlayerRepository.FAIRY_RING_MIDDLE.set(p, (middleButton(p) == 3 ? 0 : middleButton(p) + 1));
			h.actions[22] = (SimpleAction) p -> VarPlayerRepository.FAIRY_RING_MIDDLE.set(p, (middleButton(p) == 0 ? 3 : middleButton(p) - 1));
			/**
			 * Right wheel
			 */
			h.actions[23] = (SimpleAction) p -> VarPlayerRepository.FAIRY_RING_RIGHT.set(p, (rightButton(p) == 3 ? 0 : rightButton(p) + 1));
			h.actions[24] = (SimpleAction) p -> VarPlayerRepository.FAIRY_RING_RIGHT.set(p, (rightButton(p) == 0 ? 3 : rightButton(p) - 1));
			/**
			 * Teleport
			 */
		...
		InterfaceHandler.register(Interface.FAIRY_RING_TRAVEL_LOG, h -> {
			for (FairyRing fairyRing : values()) {
				h.actions[fairyRing.childId] = (SimpleAction) p -> {
					VarPlayerRepository.FAIRY_RING_LEFT.set(p, fairyRing.varpbitValues[0]);
					VarPlayerRepository.FAIRY_RING_MIDDLE.set(p, fairyRing.varpbitValues[1]);
					VarPlayerRepository.FAIRY_RING_RIGHT.set(p, fairyRing.varpbitValues[2]);
				};
			}
		});
	}
