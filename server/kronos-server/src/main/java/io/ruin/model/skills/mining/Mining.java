...
public class Mining {
	...
	private static void mine(Rock rockData, Player player, GameObject rock, int emptyId, PlayerCounter counter) {
		...
		player.startEvent(event -> {
			event.setCancelCondition(() -> player.getPosition().regionId() != rock.getPosition().regionId());
			int attempts = 0;
			while (true) {
				if (rock.getId() == emptyId) {
					player.resetAnimation();
					return;
				}
				// If the player no longer meets the requirement
				if (stat.currentLevel < rockData.levelReq) {
					player.sendMessage("You need a Mining level of " + rockData.levelReq + " to mine this rock.");
					player.privateSound(2277);
					return;
				...
				if (attempts == 0) {
					...
					if (player.petDropBonus.isDelayed())
						petOdds *= 0.8;
					petOdds *= HomeHandler.getPetDonatorBoost(player);
					if (Random.get(petOdds) == 0)
						Pet.ROCK_GOLEM.unlock(player, 0);
					...
					player.sendFilteredMessage("You manage to mine " + (rockData == Rock.GEM_ROCK ? "a " : "some ") +
						(rockData == Rock.GEM_ROCK ? ObjType.get(itemId).name.toLowerCase() : rockData.rockName) + ".");
					...
					boolean rockSaved = false;
					if (player.getPlayerPerkHandler().getActivePerks(player).contains(Perks.THE_ART_OF_MINING)) {
						...
						TheArtOfMining c = (TheArtOfMining) player.getPlayerPerkHandler().
							getActivePerks(player).get(perkIndex).getPerk(player);
						if (Random.rollPercent(c.chanceToSaveRockDepleting()))
							rockSaved = true;
					}
					if (!rockSaved && player.getPosition().regionId() != 4904) {
	...
	public static void register() {
		Object[][] oreData = {
			...
			{Rock.BASALT, 33257, 33253, PlayerCounter.MINED_BASALT},
			{Rock.URT_SALT, 33254, 33253, PlayerCounter.MINED_URT_SALT},
			{Rock.TE_SALT, 33256, 33253, PlayerCounter.MINED_TE_SALT},
			{Rock.EFH_SALT, 33255, 33253, PlayerCounter.MINED_EFH_SALT}
		};
		 ...
		 */
		Object[][] ashpile = {
			{Rock.ASHPILE, 30985, 30986, PlayerCounter.MINED_ASHPILE},
		};
		for (Object[] c : ashpile) {
			Rock rock = (Rock) c[0];
			int baseId = (Integer) c[1];
			int emptyId = (Integer) c[2];
			PlayerCounter counter = (PlayerCounter) c[3];
		...
		int[] emptyAshpile = {30986};
		for (int id : emptyAshpile) {
			ObjectAction.register(id, "mine", (player, obj) -> player.sendMessage("There is no ash currently available in this pile."));
