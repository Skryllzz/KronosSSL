...
public class Turael {
	...
	private static void assignTask(Player player) {
		SlayerMaster master = SlayerMaster.master(TURAEL);
		...
		VarPlayerRepository.SLAYER_MASTER.set(player, SlayerMaster.TURAEL_ID);
		...
		for (Pair<Integer, VarPlayerRepository> creature : SlayerUnlock.multipliable) {
			if (creature.getFirst() == def.getTaskId()) {
				if (creature.getSecond().get(player) != 0) {
					...
					System.out.println("Slayer task" +
							"" +
							"" +
							"" +
							"" +
		...
		if (!player.turaelUsed) {
			player.turaelUsed = true;
	...
	public static void handleInteraction(Player player, int option) {
		...
		switch (option) {
			case 1:
				if (talked) {
					player.dialogue(
							new NPCDialogue(TURAEL, "'Ello, and what are you after then?"),
							new OptionsDialogue(new Option("I need another assignment.", () -> {
								...
								if (!player.slayerCombatCheck) {
									player.dialogue(new NPCDialogue(TURAEL,
											...
											new OptionsDialogue(new Option("That's fine - I don't want anything too tough.", () -> {
												player.dialogue(new PlayerDialogue("That's fine - I don't want anything too tough."),
														new NPCDialogue(TURAEL, "Okay, we'll keep checking your combat level."));
									...
									player.dialogue(new NPCDialogue(TURAEL,
											...
											new OptionsDialogue(new Option("That's fine - I can handle any task.", () -> {
												player.dialogue(new PlayerDialogue("That's fine - I can handle any task."),
														new NPCDialogue(TURAEL, "That's the spirit!"));
					...
					player.dialogue(new OptionsDialogue(new Option("Who are you?", () -> {
						player.dialogue(new PlayerDialogue("Who are you?"),
								new NPCDialogue(TURAEL, "I'm one of the elite Slayer Masters."),
								new OptionsDialogue(new Option("What's a slayer?", () -> {
									player.dialogue(new PlayerDialogue("What's a slayer?"),
											new NPCDialogue(TURAEL, "Oh dear, what do they teach you in school?"),
											...
											new OptionsDialogue(new Option("Wow, can you teach me?", () -> {
												player.dialogue(new PlayerDialogue("Wow, can you teach me?"),
														new NPCDialogue(TURAEL, "Hmmm well I'm not so sure..."),
												...
												player.dialogue(new PlayerDialogue("Sounds useless to me."),
														new NPCDialogue(TURAEL, "Suit yourself."));
											})));
	...
	private static void giveTask(Player player) {
		...
		player.dialogue(
				new NPCDialogue(TURAEL,
						"Excellent, you're doing great. Your new task is to kill " + left + " "
								+ SlayerUnlock.taskName(player, task.getUid()) + "."),
				new OptionsDialogue(
						new Option("Got any tips for me?", () -> player.dialogue(
								new PlayerDialogue("Got any tips for me?"),
								new NPCDialogue(TURAEL, SlayerUnlock.tipFor(task)),
						...
						new Option("I'd like to skip this task. I have " + VarPlayerRepository.SLAYER_POINTS.get(player)
								+ " Slayer points.", () -> {
									...
									if (slayerPointsBeforeSkip >= cost) {
										...
										player.dialogue(
												new NPCDialogue(TURAEL, "Sorry, you don't have enough Slayer points to skip that task."));
									}
								}),
	...
	public static void register() {
		NPCAction.register(TURAEL, 1, (player, npc) -> handleInteraction(player, 1));
		NPCAction.register(TURAEL, "Trade", (player, npc) -> NewShopHandler.openShop(player, NewShopHandler.slayerStore));
		NPCAction.register(TURAEL, 3, (player, npc) -> handleInteraction(player, 3));
		NPCAction.register(TURAEL, 5, (player, npc) -> handleInteraction(player, 5));
	}
}
