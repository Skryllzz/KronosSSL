...
import io.ruin.model.activities.perktree.perks.TheArsonist;
...
public enum Burning {
	...
	Burning(int logId, double exp, int levelReq, int barbLevelReq, int lifeSpan, int fireId, PlayerCounter counter) {
		...
		this.pluralName = ObjType.get(logId).name.toLowerCase();
	}
	...
	private enum BarbarianBurning {
		...
		public static final BarbarianBurning[] VALUES = values();
	}
	...
	private static void burn(Player player, Item inventoryLog, Burning burning, GroundItem groundItem, Boolean barbarianBurning, int animationId) {
		...
		player.startEvent(event -> {
			...
			if (groundLog == null) {
				...
				groundLog.remove();
				DailyTasks.handleItemObtained(player, groundLog.id, StatType.Firemaking);
				PerkTaskHandler.handleGatherResource(player, 33000 + groundLog.id, 1);
				player.sendFilteredMessage("The fire catches and the logs begin to burn.");
				int xp = (int) (burning.exp * pyromancerBonus(player));
				if (player.getPlayerPerkHandler().getActivePerks(player).contains(Perks.THE_ARSONIST)) {
					int perkIndex = player.getPlayerPerkHandler().getActivePerkIndex(player, Perks.THE_ARSONIST);
					TheArsonist c = (TheArsonist) player.getPlayerPerkHandler().
	...
	private static void createFire(Burning log, GameObject obj) {
		World.startEvent(event -> {
			GameObject fire = GameObject.spawn(obj.getId(), obj.x, obj.y, obj.z, obj.getType(), obj.getDirection());
	...
	private static double lightChance(Player player, Burning log) {
		...
		if (player.getPlayerPerkHandler().getActivePerks(player).contains(Perks.THE_ARSONIST)) {
			...
			TheArsonist c = (TheArsonist) player.getPlayerPerkHandler().
				getActivePerks(player).get(perkIndex).getPerk(player);
			double diff = 1 - c.getLightSpeedBoost();
			points *= diff;
			difference *= diff;
		}
	...
	private static double pyromancerBonus(Player player) {
		...
		if (hood != null && hood.getId() == 20708 || player.getEquipment().contains(30404) || player.getEquipment().contains(30400))
			bonus += 0.4;
		if (garb != null && garb.getId() == 20704 || player.getEquipment().contains(30402) || player.getEquipment().contains(30398))
			bonus += 0.8;
		if (robe != null && robe.getId() == 20706 || player.getEquipment().contains(30403) || player.getEquipment().contains(30399))
			bonus += 0.6;
		if (boots != null && boots.getId() == 20710 || player.getEquipment().contains(30405) || player.getEquipment().contains(30401))
			bonus += 0.2;
	...
	public static void register() {
		...
		for (Burning log : values()) {
			...
			var gitemAction = (GroundItemAction) (player, groundItem, distance) -> {
				if (distance != 0) {
			 ...
			 * Ground item
			 */
			GroundItemAction.register(log.itemId, "light", gitemAction);
			ObjType.get(log.itemId).defaultItemGroundItemAction = ((player, item, groundItem, distance) -> {
				if (item.getId() != TINDER_BOX) {
					return;
				}
				gitemAction.handle(player, groundItem, distance);
			});
		}
		/**
		 * candle
