...
public class Nieve {
	...
	private static boolean assignTask(Player player) {
		...
		SlayerMaster master = SlayerMaster.master(NIEVE);

		if (master == null) {
			player.dialogue(
				new NPCDialogue(NIEVE, "Unable to assign task, i don't exist.")
			);
			return false;
		}

		VarPlayerRepository.SLAYER_MASTER.set(player, SlayerMaster.STEVE_ID);
	...
	public static void handleInteraction(Player player, int option) {
		...
		switch (option) {
			case 1:
				if (talked) {
					player.dialogue(
						new NPCDialogue(NIEVE, "'Ello, and what are you after then?"),
						new OptionsDialogue(new Option("I need another assignment.", () -> {
							...
							player.dialogue(
								...
								new NPCDialogue(NIEVE, "I have quite a few rewards you can earn, and a wide<br>variety of Slayer equipment for sale."),
							...
							if (!player.slayerCombatCheck) {
								player.dialogue(new NPCDialogue(NIEVE, "The Slayer Masters will take your combat level into<br>account when choosing tasks for you, so you shouldn't<br>get anything too hard."),
									new OptionsDialogue(new Option("That's fine - I don't want anything too tough.", () -> {
										player.dialogue(new PlayerDialogue("That's fine - I don't want anything too tough."),
											new NPCDialogue(NIEVE, "Okay, we'll keep checking your combat level."));
										...
										player.dialogue(new PlayerDialogue("Stop checking my combat level - I can take anything!"),
											new NPCDialogue(NIEVE, "Okay, from now on, all the Slayer Masters will assign<br>you anything from their lists, regardless of your combat<br>level."));
									})));
							} else {
								player.dialogue(new NPCDialogue(NIEVE, "The Slayer Masters may currently assign you any<br>task in our lists, regardless of your combat level."),
									new OptionsDialogue(new Option("That's fine - I can handle any task.", () -> {
										player.dialogue(new PlayerDialogue("That's fine - I can handle any task."),
											new NPCDialogue(NIEVE, "That's the spirit!"));
										...
										player.dialogue(new PlayerDialogue("In future, please don't give anything too rough."),
											new NPCDialogue(NIEVE, "Okay, from now on, all the Slayer Masters will take<br>your combat level into account when choosing tasks for<br>you, so you shouldn't get anything too hard."));
									})));
							}
					...
					player.dialogue(new OptionsDialogue(new Option("Who are you?", () -> {
						player.dialogue(new PlayerDialogue("Who are you?"),
							new NPCDialogue(NIEVE, "I'm one of the elite Slayer Masters."),
							new OptionsDialogue(new Option("What's a slayer?", () -> {
								player.dialogue(new PlayerDialogue("What's a slayer?"),
									new NPCDialogue(NIEVE, "Oh dear, what do they teach you in school?"),
									new PlayerDialogue("Well... er..."),
									new NPCDialogue(NIEVE, "I suppose I'll have to educate you then. A slayer is<br>someone who is trained " +
									...
									new OptionsDialogue(new Option("Wow, can you teach me?", () -> {
										player.dialogue(new PlayerDialogue("Wow, can you teach me?"),
											new NPCDialogue(NIEVE, "Hmmm well I'm not so sure..."),
											new PlayerDialogue("Pleeeaasssse!"),
											new NPCDialogue(NIEVE, "Oh okay then, you twisted my arm. You'll have to train<br>against specific groups of creatures."),
											new PlayerDialogue("Okay, what's first?").action(() -> {
												player.getInventory().addOrDrop(new Item(4155, 1));

												if (!assignTask(player)) {
													return;
												}
												...
												if (task != null) {
													...
													player.dialogue(new NPCDialogue(NIEVE, "We'll start you off hunting " + SlayerUnlock.taskName(player, task.getUid()) + ", you'll need to kill " + num + "<br>of them."));
												}
											}));
									}), new Option("Sounds useless to me", () -> {
										player.dialogue(new PlayerDialogue("Sounds useless to me."),
											new NPCDialogue(NIEVE, "Suit yourself."));
									})));
							}), new Option("Never heard of you...", () -> {
								player.dialogue(new PlayerDialogue("Never heard of you..."),
									new NPCDialogue(NIEVE, "That's because my foe never lives to tell of me. We<br>slayers are a dangerous bunch."));
							})));
					}), new Option("Er... Nothing...", () -> {
						player.dialogue(new PlayerDialogue("Er... Nothing..."));
					})));
	...
	private static void giveTask(Player player) {
		if (player.getStats().get(StatType.Slayer).fixedLevel < 50 && player.getCombat().getLevel() < 85) {
			player.dialogue(new NPCDialogue(NIEVE, "Sorry, but you're not strong enough to be taught by<br>me. Your best trainer would be " + SlayerMaster.bestMaster(player) + "."));
			return;
		}
		...
		if (left > 0) {
			...
			player.dialogue(new NPCDialogue(NIEVE, text));
			return;
		}
		...
		SlayerCreature task = SlayerCreature.lookup(VarPlayerRepository.SLAYER_TASK.get(player));
		if (task == null) {
			player.dialogue(new NPCDialogue(NIEVE, "I'm sorry, I couldn't find a task for you."));
			return;
		}
		left = VarPlayerRepository.SLAYER_TASK_AMOUNT.get(player);

		player.dialogue(
			new NPCDialogue(NIEVE, "Excellent, you're doing great. Your new task is to kill " + left + " " + SlayerUnlock.taskName(player, task.getUid()) + "."),
			new OptionsDialogue(
				new Option("Got any tips for me?", () -> player.dialogue(
					new PlayerDialogue("Got any tips for me?"),
					new NPCDialogue(NIEVE, SlayerUnlock.tipFor(task)),
				...
				new Option("I'd like to skip this task. I have " + VarPlayerRepository.SLAYER_POINTS.get(player) + " Slayer points.", () -> {
					...
					if (slayerPointsBeforeSkip >= cost) {
						...
						player.dialogue(new NPCDialogue(NIEVE, "Task skipped. You now have " + slayerPointsAfterSkip + " Slayer points remaining."));
					} else {
						player.dialogue(new NPCDialogue(NIEVE, "Sorry, you don't have enough Slayer points to skip that task."));
					}
				}),
	...
	public static void register() {
		NPCAction.register(NIEVE, 1, (player, npc) -> handleInteraction(player, 1));
		NPCAction.register(NIEVE, "Trade", (player, npc) -> NewShopHandler.openShop(player, NewShopHandler.slayerStore));
		NPCAction.register(NIEVE, 3, (player, npc) -> handleInteraction(player, 3));
		NPCAction.register(NIEVE, 5, (player, npc) -> handleInteraction(player, 5));
	}
}
