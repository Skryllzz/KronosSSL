...
import io.ruin.model.activities.perktree.perks.BobTheBuilder;
...
public abstract class Room {
	...
	private static final class TempRoom {
		...
		private void rotate(Player player, boolean clockwise) {
			...
			clear(player);
			for (RoomDefinition.VisualObject obj : type.getVisualObjects()) {
				LocType def = LocType.get(obj.id);
	...
	private Buildable[] built = new Buildable[0];
	private int rotation;
	private transient List<GameObject> objects;
	private transient DynamicChunk chunk;
	protected transient House house;
	private transient Position basePosition;
	public transient int viewerCost, viewerRotation;
	public transient int[] viewerMovedPoints;
	public transient int pointX, pointY, pointZ;
	...
	public void buildObject(Player player, int hotspotIndex, Buildable selected) {
		...
		if (selected.getUpgradeFrom() != null) {
			if (currentlyBuilt != selected.getUpgradeFrom()) {
				player.dialogue(new MessageDialogue("You must have " + StringUtils.aOrAnFormatted(StringUtils.capitalizeFirst(selected.getUpgradeFrom().name().replace("_", " ").toLowerCase())) + " built in that hotspot before you can upgrade."));
				return;
			}
		...
		player.startEvent(event -> {
			...
			selected.removeMaterials(player);
			house.calculate();
			double newExp = selected.getXP();
			if (player.getPlayerPerkHandler().getActivePerks(player).contains(Perks.BOB_THE_BUILDER)) {
				int perkIndex = player.getPlayerPerkHandler().getActivePerkIndex(player, Perks.BOB_THE_BUILDER);
				BobTheBuilder c = (BobTheBuilder) player.getPlayerPerkHandler().
			...
			player.getStats().addXp(StatType.Construction, newExp, true);
			event.delay(1);
			player.unlock();
	...
	public void renderHotspot(int hotspotIndex) {
		...
		if (built == null && !house.isBuildingMode()) { // nothing built but also not in building mode -> hide hotspot object
			...
			for (int i = 0; i < hotspot.getHotspotIds().length; i++) {
				final int index = i;
				objects.stream().
					filter(obj -> obj.getId() == hotspot.getHotspotIds()[index] || obj.originalId == hotspot.getHotspotIds()[index])
	...
	private void registerActions() {
		objects.forEach(obj -> { // door hotspots [the ones that lead into other rooms]
			if (obj == null || obj.getDef() == null || obj.getId() == -1 || obj.getDef().name == null || !obj.getDef().name.equalsIgnoreCase("door hotspot")) {
				return;
			}
			ObjectAction.register(obj, 5, (player, unused) -> {
				Room adjacent = getAdjacentRoom(Direction.fromDoorDirection(obj.getDirection()));
				if (adjacent == null) {
					if (isAdjacentInBounds(Direction.fromDoorDirection(obj.getDirection())))
						RoomDefinition.openRoomSelection(player, def -> previewBuild(player, def, Direction.fromDoorDirection(obj.getDirection())));
					else
	...
	private void render() {
		objects.stream()
			.filter(obj -> obj.getId() == 13830 || (getDefinition() == RoomDefinition.CHAPEL && IntStream.of(Hotspot.WINDOW.getHotspotIds()).anyMatch(i -> obj.getId() == i))).forEach(obj -> { // :(
				Room adjacent = getAdjacentRoom(Direction.fromDoorDirection(obj.getDirection()));
		...
		if (!house.isBuildingMode()) // doors
			objects.stream().filter(obj ->
					obj.getType() == 0
						&& onDoorLocation(obj)
						&& obj.getId() != -1
						&& obj.getId() == obj.originalId
						&& obj.getId() != house.getStyle().wallId
						&& obj.getDef().options[4] != null
						&& obj.getId() != house.getStyle().windowId)
				.forEach(obj -> {
					Room adjacent = getAdjacentRoom(Direction.fromDoorDirection(obj.getDirection()));
					if (getDefinition().isOutdoors()) // this is an outdoors 'room'... just delete the door
					...
					else if (!isAdjacentInBounds(Direction.fromDoorDirection(obj.getDirection()))) // out of bounds so instead of having a door we just place a wall so it doesn't look weird having a door to the void
						obj.setId(house.getStyle().wallId);
					else if (adjacent != null && !adjacent.getDefinition().isOutdoors()) {
						Direction dir = Direction.fromDoorDirection(obj.getDirection());
						GameObject other = Tile.getObject(-1, obj.x + dir.deltaX, obj.y + dir.deltaY, obj.z, 0, -1);
						if (other != null && (other.getId() == house.getStyle().wallId || other.getId() == house.getStyle().dungeonWallId)) // if the object adjacent to this door is a wall, that means the adjacent room isn't connected to this one on this direction,
						...
						if (VarPlayerRepository.RENDER_DOORS_MODE.get(house.getOwner()) == 2) // do not render doors
							newId = -1;
						obj.setId(newId);
						if (VarPlayerRepository.RENDER_DOORS_MODE.get(house.getOwner()) == 1) { // render door open
	...
	private void previewBuild(Player player, RoomDefinition def, Direction direction) {
		...
		player.dialogue(new OptionsDialogue(
			...
			new Option("Build", () -> {
				int x = getChunkX() + direction.deltaX;
				int y = getChunkY() + direction.deltaY;
				var tooPoor = (!player.getInventory().contains(COINS_995, def.getCost())) && !(World.isDev() && player.isAdmin());
				if (tooPoor
